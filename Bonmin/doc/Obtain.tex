...
The easiest way to obtain the released version is by downloading a compressed archive from \link{http://www.coin-or.org/Tarballs/Bonmin/}{Bonmin archive directory}. The latest release is Bonmin-1.1.0.
...
Group.}-like environments, to download the latest stable version of Bonmin (1.1) in a sub-directory, say {\tt Bonmin-1.1} 
...
\begin{colorverb}
 \noindent {\tt  svn co
https://projects.coin-or.org/svn/Bonmin/stable/1.1 Bonmin-1.1 }
\end{colorverb}

\noindent This copies all the necessary COIN-OR files to compile \Bonmin\ to
{\tt Bonmin-1.1}. To download \Bonmin\ using svn on Windows,
...
the \Bonmin\ distribution. In most Linux distribution and
...
Linear solvers are used by Ipopt. The most up-to-date information regarding the supported linear solvers and how to install them is found in \link{http://www.coin-or.org/Ipopt/documentation/node14.html}{Section 2.2} of the Ipopt manual.
...
In our experiment MA27 and MA57 usualy perform significantly better but they are freely 
available only for non-commercial, academic use. Note that all linear solver can also use Metis.
